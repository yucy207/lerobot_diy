...
@RobotConfig.register_subclass("robopanoptes")
@dataclass
class RobopanoptesRobotConfig(ManipulatorRobotConfig):
    calibration_dir: str = ".cache/calibration/robopanoptes"
    # `max_relative_target` limits the magnitude of the relative positional target vector for safety purposes.
    # Set this to a positive scalar to have the same value for all motors, or a list that is the same length as
    # the number of motors in your follower arms.
    max_relative_target: int | None = None
    ...
    leader_arms: dict[str, MotorsBusConfig] = field(
        default_factory=lambda: {
            "main": FeetechMotorsBusConfig(
                ...
                motors={
                    # name: (index, model)
                    "module1": [1, "scs_series"],
                    "module2": [2, "scs_series"],
                    "module3": [3, "scs_series"],
                    "module4": [4, "scs_series"],
                    "module5": [5, "scs_series"],
                    "module6": [6, "scs_series"],
                    "module7": [7, "scs_series"],
                    "module8": [8, "scs_series"],
                },
            ),
        }
    )
