...
LOWER_BOUND_DEGREE = -1
UPPER_BOUND_DEGREE = 201
    ...
    def read(self, data_name, motor_names: str | list[str] | None = None):
        ...
        if self.motor_models[0] != 'scs0009':
            for _ in range(NUM_READ_RETRY):
                ...
                value, comm_result, error = self.packet_handler.read1ByteTxRx(self.port_handler, 6, addr)
                print('value:',value)
                print('comm_result:',comm_result)
                print('error:',error)
                if comm == scs.COMM_SUCCESS:
                    break
        ...
        for idx in motor_ids:
            if self.motor_models[0] == 'scs0009':
                if bytes == 1:
                    comm_result, error = self.packet_handler.read1ByteTxRx(self.port_handler, idx, addr)
                elif bytes == 2:
                    comm_result, error = self.packet_handler.read2ByteTxRx(self.port_handler, idx, addr)
                print('value:',value)
                print('comm_result:',comm_result)
                print('error:',error)
                if comm_result != scs.COMM_SUCCESS or error != 0:
                    raise ConnectionError(
                        f"Read failed due to communication error on port {self.port} for group_key {group_key}: "
                        f"{self.packet_handler.getTxRxResult(comm)}"
                    )
