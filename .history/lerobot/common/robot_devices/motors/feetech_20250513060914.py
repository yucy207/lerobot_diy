...
def convert_to_bytes(value, bytes, mock=False):
    ...
    print("SCS_GETEND",scs.SCS_GETEND())
    if bytes == 1:
        data = [
...
class FeetechMotorsBus:
    ...
    def write(self, data_name, values: int | float | np.ndarray, motor_names: str | list[str] | None = None):
        ...
        if init_group:
            if data_name=='Goal_Position':
                self.group_writers[group_key] = scs.GroupSyncWrite(
                    self.port_handler, self.packet_handler, addr, bytes+4
                )
            else:
                self.group_writers[group_key] = scs.GroupSyncWrite(
                    self.port_handler, self.packet_handler, addr, bytes
        ...
        for idx, value in zip(motor_ids, values, strict=True):
            data0 = convert_to_bytes(value, bytes, self.mock)
            data1 = convert_to_bytes(0, bytes, self.mock)
            data2 = convert_to_bytes(1500, bytes, self.mock)
